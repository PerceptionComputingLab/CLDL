...
class DenseNetDist(nn.Module):
    ...
    def __init__(self, input_dim=2, growth_rate=16, block_config=(6, 12, 24, 16),
        ...
        super(DenseNetDist, self).__init__()
        ...
        for i, num_layers in enumerate(block_config):
            ...
            num_features = num_features + num_layers * growth_rate
            # Output down-sampled distribution
            up_block = nn.ConvTranspose3d(num_features, num_classes, kernel_size=2 ** i + 2,
                                          stride=2 ** i,
                                          padding=1, groups=num_classes, bias=False)
    ...
    def forward(self, x):
        first_three_features = self.features(x)
        first_three_features_bn = self.features_bn(first_three_features)
        first_pool = self.conv_pool_first(first_three_features_bn)

        out = self.dense_blocks[0](first_pool)
        up_block1 = self.upsampling_blocks[0](out)
        out = self.transit_blocks[0](out)
        ...
        up_block4 = self.upsampling_blocks[3](out)
        #
        out =  torch.cat([up_block1, up_block2, up_block3, up_block4, first_pool], 1)

        # ----------------------- classifier -----------------------
        out = self.conv_class(F.relu(self.bn_class(out)))
        # ----------------------------------------------------------

        return out, first_pool
